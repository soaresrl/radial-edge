...
class OBJFileIO {
    ...
    read(obj) {
        ...
        for (let line of lines) {
            ...
            if (parts[0] === "v") {
                this.vertices.push(new point_1.default(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3])));
            }
            else if (parts[0] === "vn") {
                this.normals.push(new point_1.default(parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3])));
            }
            else if (parts[0] === "f") {
    ...
    printLoops() {
        ...
        do {
            ...
            // do {
            if (shell_t.faceuse.orientation != definitions_1.Orientation.INSIDE) {
    ...
    printOBJ() {
        ...
        let region_t = this.model.region;
        const vertices = [];
        const vertices_points = new Map();
        const visited_vertices = new Map();
        do {
            let shell_t = region_t.shell;
            if (shell_t.faceuse.orientation != definitions_1.Orientation.INSIDE) {
                shell_t = shell_t.faceuse.mate.owningShell;
